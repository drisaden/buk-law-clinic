    ...
    .then(data => {
       ...
       // const recentItems = data.items.filter(item => item.labels.includes('Recent'));
        ...
        const populateBlog = document.getElementById('populateBlog');
         // <a href="https://adenaikeidris.com.ng/single.html?id=${item.id}" url
       //   ${item.title} title
       //${item.published.split('T')[0]} date
       //const src = item.content.match(/<img[^>]*src="([^"]*)"/)?.[1]; image
     //  ${item.content.replace(/<[^>]+>/g, '').split(' ').slice(0, 20).join(' ') content
        ...
        data.items.forEach(item => {
            ...
            <div data-aos=" " class="bg-gray-100 shadow-lg w-full rounded-lg mb-3">
                    ...
                    <div class="px-5 text-center pt-10">
                     ...
                     <a href="https://muhibqosim.github.io/buk-law-clinic/single.html?id=${item.id}" class=""> <p class="text-sm button py-3 md:text-2xl bg-red-700 text-white  mb-5 font-semibold">Read Full » </p> </a> 
                    </div>
                </div>
            `
});
